...
class RBACTestCase(TestCase):
    ...
    def test_admin_access_all_views(self):
        ...
        for url_name in ['dashboard', 'dpr_view', 'customer_po_product_details', 'supplier_po_product_details', 'rfq_details', 'customer_details', 'supplier_details', 'customer_order', 'register']:
    ...
    def test_sales_access_permissions(self):
        ...
        # Allowed views
        for url_name in ['dashboard', 'rfq_details', 'customer_order']:
            response = self.client.get(reverse(url_name))
            self.assertEqual(response.status_code, 200, f"Sales failed to access allowed view {url_name}")

        # Restricted views (403 Forbidden)
        for url_name in ['dpr_view', 'customer_po_product_details', 'supplier_po_product_details', 'customer_details', 'supplier_details', 'register']:
            response = self.client.get(reverse(url_name))
    ...
    def test_purchase_access_permissions(self):
        ...
        # Allowed views
        for url_name in ['dashboard', 'customer_order', 'dpr_view', 'customer_po_product_details', 'supplier_po_product_details']:
            response = self.client.get(reverse(url_name))
            self.assertEqual(response.status_code, 200, f"Purchase failed to access allowed view {url_name}")

        # Restricted views (403 Forbidden)
        for url_name in ['rfq_details', 'customer_details', 'supplier_details', 'register']:
            response = self.client.get(reverse(url_name))
